...
当组件的父组件重新渲染时，如果不做任何优化，那么子组件也会重新渲染。
...
## 6.强制重新渲染（函数组件）react18+版本后才有

flushSync() 可以用于强制重新渲染函数组件。不会受批处理等的影响

使用 flushSync 时，更新会被立即执行，页面会在调用时立刻反映出最新的状态。

它是强制同步更新的工具，可以用来确保某些 UI 操作在状态更新后立刻渲染。
...
function ImmediateUpdate() {
  ...
  const handleClick = () => {
    ...
    // React 会立即渲染
    console.log("当前计数：", count); // 输出会是更新后的值
  };
