...
- 2.commit 阶段
  - 不可中断
  - 先按 effectList 执行副作用，保证 DOM 和 layout 完整更新；然后整个 work-in-progress 树就完成了同步，
  - 最后切换 root.current 指向 work-in-progress 树，实现 alternate 双缓存机制，这样下一次更新可以复用 Fiber 节点，提高性能并支持中断恢复。
  ```jsx
