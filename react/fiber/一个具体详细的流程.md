...
- **调度阶段**
  - 总结：给 update 对象打 lane，同时会“标记” current Fiber 树，只标记不更新，
...
- **commit 阶段（不可中断）**
  - Before Mutation（可中断的前置阶段）
    - 执行 getSnapshotBeforeUpdate、ref 回调等
    - DOM 还没被改
  - Mutation（不可中断的 DOM 更新阶段）
    - 遍历 effect list，执行 Placement/Update/Deletion/Ref 操作
    - 这里 DOM 才真正更新
    - work-in-progress 树还没成为 current 树
    - 用户可以看到 DOM 更新了，但 Fiber 树还没切换
  - Layout（不可中断）
    - work-in-progress 树切换为 current 树
    - 执行 useLayoutEffect
    - 此阶段 DOM 已经更新
  - Passive（异步）
    - 执行 useEffect 回调
    - 浏览器已经渲染页面
    - 可以触发下一轮状态更新
      所以 useeffect 是异步的不会阻塞绘制渲染，useLayoutEffect 是同步的，会阻塞渲染绘制。
  - 副作用依赖 DOM 布局或需要同步修改 DOM 的情况，使用 useLayoutEffect。
- **完成清理**
