...
- **调度阶段**（Scheduler = Render 的“指挥官”）（决定「什么时候开始 Render」）
  ...
  ## react 自己实现的 scheduler
  - MessageChannel - setTimeout - 微任务
  ### - 为什么要自己实现
...
- **执行阶段 render(调和)-可中断**
  ...
  - beginWork 返回 null，进入 completeWork
    ...
    - 当完成当前 fiber 节点，scheduler会检查 shoudledYield/deadline.timeRemaining()，如果时间片耗尽或者有更优先的任务出现，就中断。
