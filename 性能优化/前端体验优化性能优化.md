...
- 构建缓存（cache-loader / filesystem cache）webpack的一个loader
  - Webpack 发现某个文件要通过 loader 处理cache-loader 先检查 缓存目录：
    - 缓存命中 → 直接返回处理结果
    - 缓存未命中 → 执行后续 loader
  - loader 执行完成后，将结果写入缓存
...
- 预加载
  ...
  - webpack预加载（魔法注释）不要大量用。
    - webpackPreload: true 优先级高，浏览器会立即下载，适合首屏资源
    - webpackPrefetch: true 优先级低，浏览器会在空闲时间下载，适合可能后续使用的模块
  - quickline 等工具
- ssr 浏览器渲染
- webpack相关的优化
